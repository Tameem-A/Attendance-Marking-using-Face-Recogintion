...
def contact():
    mess._show(title='Contact us', message="Please contact us on : '8886073377' ")
...
##########################SQL COnnection#############################
def connector1():
 empdata1 = pd.read_csv('StudentDetails\StudentDetails.csv', index_col=False, delimiter = ',')
 print(empdata1.head())
 ...
 try:
    conn = msql.connect(host='localhost', database='student1', user='root', password='Tameem@786')
    if conn.is_connected():
        cursor = conn.cursor()
        ...
        cursor.execute("CREATE TABLE student_data(SERIAL_NO varchar(255),ID varchar(255),NAME varchar(255),MOBILE varchar(255))")
        # loop through the data frame
        for i, row in empdata1.iterrows():
            # here %S means string values
            value = (row[0], row[2], row[4], row[6])
            sql = "INSERT INTO student1.student_data VALUES (%s,%s,%s,%s)"
...
##########################SQL COnnection#############################
def connector2():
 empdata2 = pd.read_csv("Attendance\Attendance_" + date + ".csv", index_col=False, delimiter = ',')
 print(empdata2.head())
 ...
 try:
    ...
    if conn.is_connected():
        cursor = conn.cursor()
        cursor.execute("CREATE DATABASE attendance1")
        print("Database is created")
 except Error as e:
    print("Error while connecting to MySQL", e)
 ...
 try:
    conn = msql.connect(host='localhost', database='attendance1', user='root', password='Tameem@786')
    if conn.is_connected():
        cursor = conn.cursor()
        cursor.execute("select database();")
        record = cursor.fetchone()
        ...
        print('Creating table....')
        # in the below line please pass the create table statement which you want #to create
        cursor.execute("CREATE TABLE attendance_data(Id varchar(255),Name varchar(255),Date varchar(255),Time varchar(255))")
        # loop through the data frame
        for i, row in empdata2.iterrows():
            # here %S means string values
            value = (row[0], row[2], row[4], row[6])
            sql = "INSERT INTO attendance1.attendance_data VALUES (%s,%s,%s,%s)"
            cursor.execute(sql, tuple(value))
            print("Record inserted")
            # the connection is not auto committed by default, so we must commit to save our changes
            conn.commit()
...
def sms():
    ...
    message = 'Your Ward is Present'
    mobileno = '8886073377,8088192347,9591811228'
    payload = "sender_id=FSTSMS&message=" + message + "&language=english&route=p&numbers=" + mobileno
    headers = {'authorization': "eszgMqZ0P7ISu84DcvTxYOrG2mjXb3WdafBkpEHQFtKwLlV6yCTDXhKjwzQOqGWYZvtNJAIV8cE5764u",
...
def TakeImages():
    ...
    columns = ['SERIAL NO.', '', 'ID', '', 'NAME','','MOBILE']
...
txt2.place(x=30, y=163)

lbl3 = tk.Label(frame2, text="Mobile",width=20  ,fg="black"  ,bg="#00aeff" ,font=('times', 17, ' bold '))
lbl3.place(x=80, y=190)

txt3 = tk.Entry(frame2,width=32 ,fg="black",font=('times', 15, ' bold ')  )
txt3.place(x=30, y=220)
...
clearButton2.place(x=335, y=163)
clearButton3 = tk.Button(frame2, text="Clear", command=clear2  ,fg="black"  ,bg="#ea2a2a"  ,width=11 , activebackground = "white" ,font=('times', 11, ' bold '))
clearButton3.place(x=335, y=219)
